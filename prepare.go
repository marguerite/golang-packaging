...
func prepareBuildEnvironment(args []string, module bool) {
	...
	if module {
		fmt.Println("Moduleaware mode detected, copying vendor to mod")
		...
		fmt.Printf("Faking %s with BuildRequires and vendor directory\n", filepath.Join(builddir, "go.mod"))

		fakeGoMod(filepath.Join(builddir, "go.mod"))
